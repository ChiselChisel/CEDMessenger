...
class CEBProfessional:
    ...
    def __init__(self):
        ...
        # Server-specific attributes for multi-client support

        self.ceb_server_socket = None

        self.ceb_client_connections = []  # List of (socket, username, address) tuples

        self.ceb_server_running = False

        self.ceb_clients_lock = threading.Lock()  # Thread safety for client list
    ...
    def ceb_toggle_afk(self):
        ...
        if self.ceb_hosting:
            ...
            self.ceb_broadcast_to_clients(ceb_data.encode())

        elif self.ceb_socket_conn:

            try:

                self.ceb_socket_conn.send(ceb_data.encode())
    ...
    def ceb_quit_session(self):
        ...
        if messagebox.askyesno("Quit Session", "Are you sure you want to end this session?"):
            ...
            if self.ceb_hosting:
                ...
                with self.ceb_clients_lock:

                    for client_sock, _, _ in self.ceb_client_connections:
                    ...
                    self.ceb_client_connections.clear()

               

                if self.ceb_server_socket:

                    try:

                        self.ceb_server_socket.close()
    ...
    def ceb_show_emoji_picker(self):
        ...
        for emoji in ceb_emojis:
            ...
            btn = tk.Button(
                ...
                command=lambda e=emoji: self.ceb_send_emoji(e, ceb_emoji_window)
    ...
    def ceb_send_emoji(self, emoji, window):

        """CEB Send emoji as a message"""

        window.destroy()

       

        ceb_data = f"EMOJI:{emoji}"
        ...
        self.ceb_add_message(emoji, "ceb_own_msg")
        ...
        if self.ceb_hosting:
            ...
            try:
                ...
                self.ceb_add_message("Failed to send emoji.", "ceb_system")
    ...
    def ceb_initialize_connection(self, host_mode):
        ...
        if not host_mode:
            ...
            # Ask for server IP to connect to

            server_ip = simpledialog.askstring("Join Session",
            ...
            if not server_ip:
                ...
                messagebox.showwarning("Connection Required", "Please enter a server IP.")
        ...
        if host_mode:
            ...
            threading.Thread(target=self.ceb_join_session, args=(server_ip,), daemon=True).start()
    ...
    def ceb_send_message(self):
        ...
        # Check if connected (either hosting or client)

        if not self.ceb_hosting and not self.ceb_socket_conn:
    ...
    def ceb_broadcast_to_clients(self, data):

        """CEB Broadcast message to all connected clients"""

        with self.ceb_clients_lock:

            disconnected = []

            for client_sock, username, addr in self.ceb_client_connections:
                ...
                try:
                    ...
                    disconnected.append((client_sock, username, addr))
            ...
            for client in disconnected:
                ...
                if client in self.ceb_client_connections:

                    self.ceb_client_connections.remove(client)

                    if client[1] in self.ceb_connected_users:

                        self.ceb_connected_users.remove(client[1])

                        self.ceb_user_status.pop(client[1], None)

                        self.ceb_add_message(f"{client[1]} disconnected", "ceb_system")

                        self.ceb_update_users_list()

   

    def ceb_parse_protocol_message(self, data, sender_username):

        """CEB Parse TYPE:CONTENT protocol message"""
        ...
        try:
            ...
            msg_type, content = decoded.split(":", 1)
            ...
            if msg_type == "MSG":
                ...
                self.ceb_add_message(sender_username, "ceb_other_name")

                self.ceb_add_message(content, "ceb_other_msg")

                self.ceb_typing_label.config(text="")

                if sender_username in self.ceb_user_status:

                    self.ceb_user_status[sender_username] = "online"
                ...
                self.ceb_add_message(f"{sender_username} is now away", "ceb_status")

                self.ceb_user_status[sender_username] = "afk"
                ...
                if content == "start":
                    ...
                    self.ceb_user_status[sender_username] = "typing"

                    self.ceb_typing_label.config(text=f"{sender_username} is typing...")
                ...
                if sender_username not in self.ceb_connected_users:

                    self.ceb_connected_users.append(sender_username)

                    self.ceb_user_status[sender_username] = "online"

                    self.ceb_update_users_list()

                    self.ceb_add_message(f"{sender_username} joined the session", "ceb_system")
    ...
    def ceb_host_session(self):
        ...
        """CEB Host multi-client server session"""

        try:

            self.ceb_server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

            self.ceb_server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)

            self.ceb_server_socket.bind((CEB_HOST, CEB_PORT))

            self.ceb_server_socket.listen(5)  # Allow up to 5 queued connections

           

            self.ceb_server_running = True
            ...
            self.ceb_status_indicator.config(text="● Hosting", fg=CEB_SUCCESS_GREEN)

           

            # Accept multiple clients

            while self.ceb_server_running and self.ceb_active_session:

                try:

                    self.ceb_server_socket.settimeout(1.0)  # Timeout to check flags

                    ceb_conn, ceb_addr = self.ceb_server_socket.accept()
                    ...
                    client_thread = threading.Thread(
                        ...
                        target=self.ceb_handle_client,

                        args=(ceb_conn, ceb_addr),
    ...
    def ceb_handle_client(self, client_sock, addr):

        """CEB Handle individual client connection"""
        ...
        try:
            ...
            self.ceb_add_message(f"Participant connected from {addr[0]}", "ceb_system")
                ...
                if ":" in decoded:
                    ...
                    if msg_type.upper() == "USERNAME":
                        ...
                        with self.ceb_clients_lock:
                            ...
                            self.ceb_client_connections.append((client_sock, username, addr))

                       

                        if username not in self.ceb_connected_users:

                            self.ceb_connected_users.append(username)

                            self.ceb_user_status[username] = "online"

                            self.ceb_update_users_list()

                            self.ceb_add_message(f"{username} joined the session", "ceb_system")

                       

                        # Send our username to the client

                        client_sock.send(f"USERNAME:{self.ceb_user_name}".encode())

                       

                        # Notify other clients

                        self.ceb_broadcast_to_clients(f"USERNAME:{username}".encode())
            ...
            while self.ceb_server_running and self.ceb_active_session:
                ...
                if username:
                    ...
                    self.ceb_parse_protocol_message(data, username)
                    ...
                    with self.ceb_clients_lock:
                        ...
                        for other_sock, other_user, other_addr in self.ceb_client_connections:
            ...
            with self.ceb_clients_lock:
                ...
                for client in self.ceb_client_connections:
            ...
            if username:
                ...
                if username in self.ceb_connected_users:

                    self.ceb_connected_users.remove(username)

                    self.ceb_user_status.pop(username, None)

                    self.ceb_add_message(f"{username} disconnected", "ceb_system")
    ...
    def ceb_join_session(self, server_ip):
        ...
        try:
            ...
            ceb_client.connect((server_ip, CEB_PORT))

            self.ceb_socket_conn = ceb_client

           

            self.ceb_add_message(f"Connected to session at {server_ip}:{CEB_PORT}", "ceb_system")
            ...
            self.ceb_socket_conn.send(f"USERNAME:{self.ceb_user_name}".encode())
    ...
    def ceb_receive_messages(self):
        ...
        """CEB Receive messages loop for client"""
        ...
        while self.ceb_active_session:
            ...
            try:
                ...
                if not ceb_data:
                    ...
                    self.ceb_add_message("Connection closed by server.", "ceb_system")
                ...
                if ":" in decoded:
                    ...
                    if msg_type.upper() == "USERNAME":
                        ...
                        if content not in self.ceb_connected_users and content != self.ceb_user_name:

                            self.ceb_connected_users.append(content)

                            self.ceb_user_status[content] = "online"

                            self.ceb_update_users_list()

                            self.ceb_add_message(f"{content} joined the session", "ceb_system")

                    else:

                        # Try to extract username from message context or use "Remote User"

                        # In multi-user scenario, messages should include sender info

                        # For now, we'll use the first connected user or "User"

                        sender = self.ceb_connected_users[0] if self.ceb_connected_users else "User"

                        self.ceb_parse_protocol_message(ceb_data, sender)
